...
echo "=== Test 15: rename moves file and persists across remount ==="
mount_rw
echo "data" > "$MOUNTPOINT/before_rename.txt"
mv "$MOUNTPOINT/before_rename.txt" "$MOUNTPOINT/after_rename.txt"
unmount_wait
mount_rw
[ ! -f "$MOUNTPOINT/before_rename.txt" ] || { echo "FAIL: source still exists"; unmount_wait; exit 1; }
CONTENT="$(cat "$MOUNTPOINT/after_rename.txt")"
[ "$CONTENT" = "data" ] || { echo "FAIL: renamed content: $CONTENT"; unmount_wait; exit 1; }
unmount_wait
echo "PASS"
